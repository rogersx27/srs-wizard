...
const LABELS: Record<QualityFindingType, string> = {
  vagueness: "Vaguedad",
  duplicate: "Posible duplicado",
  missing_priority: "Sin prioridad",
};

const STYLES: Record<QualityFindingType, string> = {
...
export function QualityFindingBadge({ type }: { type: QualityFindingType }) {
  return (
    <span className={clsx("inline-flex rounded-full px-2.5 py-0.5 text-xs font-medium", STYLES[type])}>
      {LABELS[type]}
    </span>
  );
}
