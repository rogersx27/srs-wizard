...
- Las funciones de IA se integran únicamente a través del puerto `IAiAssistant` (`src/domain/ports/`), implementado por adaptadores en `src/infrastructure/ai/` (ej. `GeminiAiAssistant`). Nunca se llama al SDK de un proveedor directamente desde `application` o `domain` — así se puede cambiar de proveedor sin tocar esas capas.
...
- Los resultados de IA se cachean en la tabla `AiCache` (`src/infrastructure/ai/cachedCompute.ts`, vía `IAiCacheRepository`), con clave `(projectId, kind)` e invalidación por hash del contenido que los generó. **Solo se cachea un resultado exitoso** (`cacheable: true`) — un fallback/degradado nunca se guarda, para que la siguiente consulta reintente en vez de quedar atascada sirviendo el fallback indefinidamente.
