...
- Las funciones de IA se integran únicamente a través de los puertos `IAiAssistant` e `IAiEvaluator` (`src/domain/ports/`). Los puertos, los adaptadores de cada proveedor (Gemini, Anthropic, OpenAI, Vercel AI Gateway, Jev) y las utilidades de timeout/caché viven en la librería [`@rogersx27/ai-ports`](https://github.com/rogersx27/ai-ports), instalada desde GitHub con un tag fijo (`github:rogersx27/ai-ports#vX.Y.Z`). `src/domain/ports/` solo re-exporta sus tipos, y el único lugar que construye adaptadores es `src/container/di.ts`. Nunca se llama al SDK de un proveedor directamente desde `application` o `domain` — así se puede cambiar de proveedor sin tocar esas capas. Un cambio en los adaptadores se hace en `ai-ports`, se publica como nuevo tag y aquí solo se sube la versión.
- Ninguna funcionalidad de IA puede bloquear un flujo existente: si el proveedor falla, no responde a tiempo o no hay API key configurada (`NullAiAssistant`), el código debe degradar al comportamiento mecánico anterior (texto crudo del cliente), nunca lanzar un error que rompa la página.
- `pnpm test` corre con `node --test`, que usa el *type-stripping* nativo de Node (sin transformación): los *parameter properties* de TypeScript (`constructor(private readonly x: Foo)`), los `enum` y otras construcciones no erasables rompen la carga de cualquier archivo `.ts` importado (directa o transitivamente) por un test. El código de dominio/entidades puede seguir usando parameter properties porque no se importa directo en los tests; la lógica que sí se testea (ej. `narrativeRewriter.ts`) usa interfaces angostas y evita esas construcciones a propósito.
- **Imports en módulos cubiertos por `node --test`:** los imports de ejecución entre archivos locales que los tests cargan directa o transitivamente deben usar rutas relativas con extensión `.ts` (ej. `../../wizard-catalog/ieee830Mapping.ts`). Node no resuelve el alias `@/` ni agrega la extensión al ejecutar estos tests; no usamos un loader adicional. Los `import type` pueden conservar `@/` o rutas sin extensión porque se eliminan antes de ejecutar. En módulos que solo carga Next.js se mantiene el estilo existente (`@/` o rutas relativas sin extensión); los imports de paquetes conservan su nombre normal.
- Los resultados de IA se cachean en la tabla `AiCache` (`withCache` de `@rogersx27/ai-ports`, vía `IAiCacheRepository` implementado por `PrismaAiCacheRepository`), con clave `(projectId, kind)` e invalidación por hash del contenido que los generó. **Solo se cachea un resultado exitoso** (`cacheable: true`) — un fallback/degradado nunca se guarda, para que la siguiente consulta reintente en vez de quedar atascada sirviendo el fallback indefinidamente.
